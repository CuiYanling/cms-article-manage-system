<template>
    ...
    <el-dialog v-model="dialogVisible" title="编辑角色">
        <el-form ref="formRef" :rules="rules" :model="form">
            <el-form-item label="角色名称" prop="name" status-icon label-width="80px">
    ...
    <!-- <el-dialog v-model="menuside" title="设置角色菜单" class="menuside" append-to-body>
    ...
    <el-drawer v-model="menuside" title="设置角色菜单" class="menuside" append-to-body size="24%">
        <el-tree ref="treeRef" :data="data" show-checkbox node-key="id" :default-expanded-keys="unfoldarr"
            :default-checked-keys="checkedarr" :props="defaultProps" />
        <template #footer>
            <div style="flex: auto">
...
let handleSureadd = (formEl) => {
    formEl.validate(async (valid) => {
        if (valid) {
            let { status, msg, data } = await Role.add({ ...formadd.value });
            // console.log(status);
            console.log(formadd.value.name);
            console.log({ ...formadd.value });
            if (status) {
                personlist.value.push({ ...formadd.value });
                //关闭窗口
                dialogVisibleadd.value = false;
                //重新获取，可以得到id
                loadList();
...
watch(isId, async (news, old) => {
    checkedarr.value = [];
    let arr = [];
    let { data: datame, } = await Role.RoleMenu({ id: news, type: "flat" });
...
let handleMenu = async (id, index) => {
    isId.value = id;
    let { data: datas, } = await Role.RoleMenu({ id: 1, type: "tree" });
    data.value = [{ id: 1, name: '全部菜单', children: datas }]
    // console.log(data.value);
    let { data: dataunfold, } = await Role.RoleMenu({ id: 1, type: "flat" });
...
//     let { status, msg } = await Role.MenuAdd({ id: isId.value, menus: treeRef.value.getCheckedKeys(true) })
//     if (status) {
//         ElMessage.success(msg);
//         isId.value = 0
//     } else {
