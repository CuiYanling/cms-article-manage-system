<template>
    <el-card class="box-card">
        <template #header>
            <div class="card-header">
                <div>标签列表</div>
                <div>
                    <el-button icon="CirclePlus" @click="handleadd" type="primary" plain>添加标签</el-button>
                </div>
            </div>
        </template>
    ...
    <!-- 添加标签按钮 dialog 弹窗 -->
    <el-dialog v-model="adddialogVisible" title="添加标签">
        <el-form ref="formaddRef" :rules="rules" :model="formadd">
        ...
        <template #footer>
            <span class="dialog-footer">
                ...
                <el-button type="primary" @click="handleAdd(formaddRef)">添加</el-button>
            </span>
        </template>
    </el-dialog>
...
// 弹窗中取消按钮
function handleAddCancle() {
    adddialogVisible.value = false;
}
//获取form组件实例---校验登录
let formaddRef = ref();
// 弹窗中添加按钮（发送axios）
let handleAdd = (formAddEl) => {
    //表单验证
    formAddEl.validate(async (valid) => {
        if (valid) {
            let { status, msg, data } = await Article.createtag({ ...formadd.value });
