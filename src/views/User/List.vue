    ...
    <el-card class="box-card">
        ...
        <el-table :data="userList" style="width: 100%">
            ...
            <el-table-column prop="operation" label="操作" width="200" #default="scope">
                <el-row>
                    <el-button :type="scope.row.usable===1?'info':'success'" plain prop="usable"
                        @click="handleopen(scope.row.id,scope.row.usable,scope.$index)">
                        <el-icon>
                            <DeleteFilled />
                        </el-icon>
...
//禁用。启用。 按钮
function handleopen(id, usable, i) {
    ElMessageBox.confirm(
        "此操作将" + (usable === 1 ? '禁用' : '启用') + "该公告，是否继续?",
        {
            type: 'warning',
            cancelButtonText: '取消',
